...
// Firebase otp Request (requires client-side reCAPTCHA/SafetyNet — cannot be called from server directly)
...
//     if (!user) {
//         user = await prisma.user.create({ data: { phone, ...(fcmToken && { fcmToken }) } })
//     } else if (fcmToken) {
//         user = await prisma.user.update({ where: { phone }, data: { fcmToken } })
//     }
//
//     try {
...
//             data: { phone, sessionInfo, expiresAt: new Date(Date.now() + 5 * 60 * 1000), verified: false }
//         });
//         if (process.env.NODE_ENV !== "production") {
//             return res.json({ success: true, sessionInfo, expiresIn: 300 })
//         }
