...
export const createInvitation = async ({
  ...
  if (tournamentId) {
    ...
    if (type === "PLAYER") {
      ...
      const alreadyParticipant = await prisma.tournamentParticipant.findFirst({
        where: {
          tournamentId,
          OR: [
            { playerId },
  ...
  try {
    ...
    if (type === "TEAM" && teamId) {
      ...
      const tokens = admins.map((m) => m.user?.fcmToken).filter(Boolean);
    ...
    console.error(
      "🔕 Notification send failed (non-critical):",
      notifError.message,
    );
  }
...
export const acceptInvitation = async (invitationId, userId) => {
  return prisma.$transaction(async (tx) => {
    ...
    if (invite.tournamentId) {
      ...
      if (invite.type === "PLAYER") {
        ...
        if (gameType === "DOUBLES") {
          ...
          await ensureTeamMember(tx, invite.teamId, userId, "PLAYER");
    ...
    if (invite.type === "PLAYER" && invite.targetTeamId) {
      await ensureTeamMember(tx, invite.targetTeamId, userId, "PLAYER");
    }
...
export const listInvitationsByUserId = async (userId, status) => {
    const adminTeams = await prisma.teamMember.findMany({
        where: {
            userId,
            role: {
